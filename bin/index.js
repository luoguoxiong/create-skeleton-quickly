...
function getConfig() {
  ...
  if (!fs.existsSync(confFile)) {
    return utils.log.error(`please run 'create-skeleton init' to initialize a config file`, 1);
  }
...
program
  ...
  .action(function (env, options) {
    ...
    if (fs.existsSync(confFile)) {
      return console.log(
        `\n[${conf.filename}] had been created! you can edit it and then run 'create-skeleton start'\n`,
      );
    }
    fs.writeFile(path.resolve(currDir, conf.filename), conf.getTemplate(), (err) => {
      if (err) throw err;
      console.log(
        `\n[${conf.filename}] had been created! now, you can edit it and then run 'create-skeleton start'\n`,
      );
    });
  });
